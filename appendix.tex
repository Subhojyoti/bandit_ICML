...
where $b\geq \sqrt{\frac{K}{\gamma T}}$, $C_1(x) = \frac{2^{1+4x}x^{2x}}{\psi^{x}}$, $C_2(x) = \frac{2^{2x+\frac{3}{2}}x^{2x}}{\psi^{x}}$, $\rho_{a}\leq \rho_{s}$, $\gamma$ is a constant and $A_{s^{*}}$ is the subset of arms in cluster $s^{*}$ containing optimal arm $a^{*}$.
...
Since each round now consist of $|B_{m}|n_{m}$ timesteps and since at every timestep the arm elimination condition is being checked, following a parallel arguement as in Case a1, Theorem $\ref{Result:Theorem:1}$ we can show that when $z_{i} = n_{m_{i}}$ then \\ $c_{i} = c_{m_{i}} = \sqrt{\dfrac{\rho_{a}\log{(\psi T\epsilon_{m}^{2})}}{2 n_{m{i}}}} = \sqrt{\rho_{a}\epsilon_{m_{i}+1}} < \frac{\Delta_{i}}{4}$, since $z_{i} = n_{m_{i}}=\frac{2\log{(\psi T\epsilon_{m_{i}}^{2})}}{\epsilon_{m_{i}}}$ and $\rho_{a}\in (0,1]$.
...
\item each sub-optimal arm ${i}\in A^{'}$ is  eliminated on or before $\max (m_{i},g_{s_{k}})$ and hence  pulled not more than $z_{i}$ number of times. But according to the condition of Case a1, $z_{i} \leq n_{m_{i}}$ number of times.\\
\item each sub-optimal cluster $s_k \in \check A$ is eliminated on or before $\max (m_{i},g_{s_{k}})$ and hence  pulled not more than $n_{a_{\max_{s_{k}}}}$ number of times. But again according to condition Case a2,  $z_{a_{\max_{s_{k}}}} \leq n_{g_{s_{k}}}$ number of times.
\end{inparaenum}
...
In this case the optimal arm ${*}\in s^{*}$ is not eliminated, also $s^{*}$ is not eliminated. So, for all sub-optimal arms $i$ in $A_{s^*}^{'}$ which gets eliminated on or before $\max \lbrace m_{i},g_{s_{k}} \rbrace$ will get pulled no less than $z_{i}\leq \bigg\lceil\dfrac{2\log{(\psi T\epsilon_{m_{i}}^{2})}}{\epsilon_{m_{i}}}\bigg\rceil$ number of times, which leads to the following bound the contribution to the expected regret, as in Case $b1$:
% 
% \begin{align*}
...
\State {\bf Initialization:} Set $m:=0$, $B_{0}:=A$, $S_0 = S$, $\epsilon_{0}:=1$, $M=\big \lfloor \dfrac{1}{2}\log_{2} \dfrac{7T}{K}\big\rfloor$, $n_{0}=\bigg\lceil\dfrac{2\log{(\psi T\epsilon_{0}^{2})}}{\epsilon_{0}}\bigg\rceil$, $\ell_{0}:=2$ and  $N_{0}=Kn_{0}$.
...
\begin{figure}[!tbp]
    ...
    \subfigure[0.32\textwidth][Experiment $3$: Experiment $3$: $20$ to $90$ Bernoulli-distributed arms with $r_{i_{{i}\neq {*}}}=0.05$ and $r^{*}=0.1$. ]
    {
        ...
        % UCB
		\addplot table{results/NewExpt/Expt3_1/EclUCB20_90.txt};
		\addplot table{results/NewExpt/Expt3_1/MOSS20_90.txt};
		\addplot table{results/NewExpt/Expt3_1/OCUCB20_90.txt};
      	\legend{EClusUCBA,MOSS,OCUCB}
      	\end{axis}
        \end{tikzpicture}
        \label{fig:3}
    }
    ...
    \subfigure[0.32\textwidth][Experiment $4$: Cumulative regret for ClusUCB for various clusters]
    {
		...
		\addplot table{results/NewExpt/Expt4/plotFinalEclUCB011.txt};
		\addplot table{results/NewExpt/Expt4/plotFinalAclUCB011.txt};
    ...
    \subfigure[0.32\textwidth][Experiment $5$: Error Percentage of EClusUCB and CCB]
    {
        ...
        %clip mode=individual,grid,grid style={gray!30},
  		legend style={at={(0.5,1.3)},anchor=north, legend columns=3} ]
      	% UCB
      	\addplot table{results/NewExpt/Expt5/EclUCB01_comp_subsampled.txt};
...
\end{figure}
	...
	The fourth experiment is conducted the to show that our choice of $p=\lceil\frac{K}{\log K}\rceil$ which we use to reduce the elimination error, is indeed close to optimal. The experiment is performed over a testbed having $30$ Bernoulli-distributed arms with $r_{i_{:{{i}\neq {*}}}}=0.07,\forall i\in A$ and $r^{*}=0.1$. In Figure \ref{fig:4}, we report the cumulative regret over $T=80000$ timesteps averaged over $100$ independent runs plotted against the number of clusters $p=1$ to $\frac{K}{2}$. Also, in this experiment we take $\psi = \log T$, $\rho_a=0.5$ and $\rho_{s}=0.5$ for AClusUCBA and ECLusUCBA whereas AClusUCBA and EClusUCBA have $\psi=\frac{T}{\log K}$. We see that for $p=\lceil\frac{K}{\log K}\rceil=9$, the cumulative regret of EClusuCB and EClusUCBA are almost the lowest over the entire range of clusters. Also, we show cumulative regret for AClusUCB or AClusUCBA (which does not have $p$ as parameter) as a straight line, constant over the number of clusters. AClusUCB or AClusUCBA consistently performs poorly as compared to EClusUCB or EClusUCBA. We conjecture that this happens because AClusUCB conducts a significant amount of initial exploration to find the number of clusters as opposed to EClusUCB which has an uniform clustering scheme from the very start. 
