...
\end{align*}
for all $b\geq\sqrt{\frac{K}{7T}}$. In the above, $C_1, C_2$ are as defined in Theorem \ref{Result:Theorem:1}.
...
EClusucb has the same regret guarantees as ClusUCB and its regret upper bound is shown in Theorem \ref{Result:Theorem:2} in Appendix \ref{App:EClusUCB}. Also EClusUCB is an anytime algorithm and its simple regret guarantee is weaker than CCB(\cite{liu2016modification}) and that is shown in Appendix \ref{App:SR_EClusUCB} in Corollary \ref{App:Corollary:3}.
